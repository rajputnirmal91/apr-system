...
import { useGetRatingListNewQuery } from '@project/Store/Api/Admin/Ratings/rating'

import './Help.scss'

function Help() {
  const navigate = useNavigate()
  const { data: Rating } = useGetRatingListNewQuery({
    page: 1,
    limit: 10,
    search: '',
    order_by: 'asc',
  ...
  return (
    // <Container className="helpUnitHead">
    <div className="padding2480">
      <button
        onClick={() => navigate(-1)}
            ...
            {Rating?.ratings && Rating?.ratings.length > 0
              ? Rating?.ratings.map((data) => (
                  <h3 className="font16 font400 fontOnest mb-0 mt-2">
                    <span className="primaryColor">{data.rating_name} -</span> (
                    {data.rating_title}):{' '}
